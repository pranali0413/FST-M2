...
inputFile1 = LOAD 'hdfs:///user/nikita/episodeIV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile2 = LOAD 'hdfs:///user/nikita/episodeV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile3 = LOAD 'hdfs:///user/nikita/episodeVI_dialogues.txt' using PigStorage('\t') AS (name,line);
...
STORE Sorted_Output INTO 'hdfs:///user/nikita/Output_Characters';
